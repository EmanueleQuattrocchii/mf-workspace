...
export class EventDetailComponent implements OnInit{
  ...
  constructor(public mfs: MfeService, public gn: GeneralService,public auth: AuthService,private router: Router,private resService: ReservationService,private sesService: SessionService,private headerService: HeaderService) { }
  ...
  ngOnInit() {
    ...
    this.mfs.event= this.gn.eventDetail;
  }

  register(session: gameSessionModel) {
    this.router.navigate(["subscription"]);
    if (this.auth.isLogged) {
      if(this.mfs.isSessionDetail){
      ...
      this.gn.isLoading = true;
        this.resService.createReservation(session.sessionId).then(() => {
          ...
          this.gn.isOverlayOn$.next(true);
            ...
            const sessionIndex = this.mfs.event?.sessions?.findIndex(s => s.sessionId === session.sessionId);
            if (sessionIndex !== undefined && sessionIndex !== -1 && this.mfs.event?.sessions) {
              this.mfs.event.sessions[sessionIndex] = updatedSession;
            }
            });
  ...
  closeSessionDetail() {
    this.mfs.isSessionDetail = false;
    this.gn.isOverlayOn$.next(false);
    this.mfs.sessionDetail = undefined;
  }
